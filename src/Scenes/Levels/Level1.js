...
export default class Level1 extends Phaser.Scene {
  static count = 0;
  static addNumber(){
    Level1.count = (Level1.getNumber() + 1)%5 ;
  }
  static getNumber(){
    return Level1.count;
  }
  constructor() {
    super('levelFirst' + (Level1.getNumber() + 1));
    Level1.addNumber();
  }
  preload(){
  ...
  create() {
    ...
    //Backgrounds.
    this.add.image(-2000, 500, 'bg1').setScale(2).setScrollFactor(0.5,0.1).setDepth(-300);
    ...
    if(this.map.getObjectLayer("Chest_Layer") != null)
      this.map.getObjectLayer("Chest_Layer").objects.forEach(point => {
        if(point.name == "tutorialSpecial")
          new InteractableChest(this, point.x, point.y, 100 ,20000);
        else
    ...
    for(var i=0; i<this.goalArray.length; i++){
      this.goalArray[i].initGoal('levelSecond', Level2);
    }
  ...
  pauseGame(){
    ...
    this.scene.pause('levelFirst' + (Level1.getNumber()));
  }
